    ...
    <div className="converter-grid">
      ...
      >
        <div className="drop-topline">
          ...
          <span className="status-pill">{loading ? "Processing" : "Ready"}</span>
        </div>
        ...
        {isDragging && <div className="drag-overlay">پوشه را اینجا رها کنید</div>}
      </section>
      ...
      <aside className="result-panel">
        ...
        {stats && !error && (
          <div className="result-card success">
            ...
            <p>
              فایل‌های <strong>{getFileName(stats.folderName)}</strong> در
    ...
    <section className="page-card ai-panel">
      <div className="ai-content">
        ...
        <div className="ai-log">
          ...
          <div>
            {aiLogs.length === 0 ? (
              <p>هنوز لاگی ثبت نشده.</p>
            ) : (
              aiLogs.slice(-6).map((line, index) => <p key={`${line}-${index}`}>{line}</p>)
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <section className="page-card speech-page">
      <div className="speech-grid">
        <label htmlFor="speech-text" className="field-label">
        ...
        <div className="actions">
          <button type="button" className="btn btn-primary" onClick={handleSpeak} disabled={speaking}>
            <span>{speaking ? "در حال خواندن..." : "خواندن متن"}</span>
          </button>

          <button type="button" className="btn btn-secondary" onClick={handleStop} disabled={!speaking}>
            <span>توقف</span>
          </button>
        </div>
